(()=>{
  ...
  function simulation(){return window.AsymptaWorldSimulation||window.ASYMPTA_SIMULATION_V10||null}
  function setSimulationRunning(running){
    const badge=$("liveSimBadge");
    ...
    if((running&&paused)||(!running&&!paused))badge.click();
    return {ok:true,running};
  }

  function parseTask(rest){
    const m=rest.match(/^@([^\s]+)\s+(.+)$/);return m?{assignee:m[1],body:m[2]}:{assignee:"Unassigned",body:rest};
  }
  function parseCredit(rest){
    const m=rest.match(/^@?([^\s]+)\s+([0-9]+(?:\.[0-9]+)?)\s*(.*)$/);return m?{recipient:m[1],amount:Number(m[2]),note:m[3]}:null;
  }
  ...
  async function execute(raw,{source=HUMAN,name}={}){
    ...
    const actor=actorName(source,name);
    const firstSpace=command.indexOf(" "),head=(firstSpace<0?command:command.slice(0,firstSpace)).toLowerCase(),rest=firstSpace<0?"":clean(command.slice(firstSpace+1));
    const cmd=head.startsWith("/")?head.slice(1):head;
    const A=api();
    if(!A&&!["help","focus","continue","context","world","pause","resume","clear","tools"].includes(cmd))return {ok:false,error:"activity_kernel_unavailable"};

    if(cmd==="help")return {ok:true,text:"/post text · /task @name text · /proposal text · /share URL note · /credit @name amount note · /activities [type] · /ledger · /context · /focus thought-id · /continue text (Human) · /world · /pause · /resume · /tools · /clear"};
  ...
  function registerTool(){
    ...
    try{mc.registerTool({
      name:"execute_world_command",
      description:"Execute an Asympta Terminal command as an Agent. Commands share the same activity kernel and visible terminal history as Human commands. Use structured WebMCP tools for Agent thought continuations; terminal commands are ideal for activities, tasks, resources, simulated World Credits, navigation, and World status.",
      inputSchema:{type:"object",properties:{command:{type:"string",minLength:1,maxLength:1400},agent_name:{type:"string"}},required:["command"]},
      execute:async({command,agent_name="Agent"})=>{
        append(AGENT,`› ${command}`,"command");const result=await execute(command,{source:AGENT,name:agent_name});append("system",result.text||resultText(result),result.ok===false?"error":"success");return result;
      }
    })}catch(err){console.warn("Terminal WebMCP registration failed",err)}
  }
